...
void AppendProtoDebugString(
  ...
  o->AppendNumericIfNotZero("all_start_nanos", msg.all_start_nanos());
  o->AppendNumericIfNotZero("op_start_rel_nanos", msg.op_start_rel_nanos());
  o->AppendNumericIfNotZero("op_end_rel_nanos", msg.op_end_rel_nanos());
  o->AppendNumericIfNotZero("all_end_rel_nanos", msg.all_end_rel_nanos());
  o->AppendNumericIfNotZero("scheduled_nanos", msg.scheduled_nanos());
}
...
bool ProtoParseFromScanner(
  ...
  while(true) {
    ...
    else if (identifier == "all_start_nanos") {
      if (has_seen[12]) return false;
      has_seen[12] = true;
      int64 value;
      if (!parsed_colon || !::tensorflow::strings::ProtoParseNumericFromScanner(scanner, &value)) return false;
      msg->set_all_start_nanos(value);
    }
    else if (identifier == "op_start_rel_nanos") {
      if (has_seen[13]) return false;
      has_seen[13] = true;
      int64 value;
      if (!parsed_colon || !::tensorflow::strings::ProtoParseNumericFromScanner(scanner, &value)) return false;
      msg->set_op_start_rel_nanos(value);
    }
    else if (identifier == "op_end_rel_nanos") {
      if (has_seen[14]) return false;
      has_seen[14] = true;
      int64 value;
      if (!parsed_colon || !::tensorflow::strings::ProtoParseNumericFromScanner(scanner, &value)) return false;
      msg->set_op_end_rel_nanos(value);
    }
    else if (identifier == "all_end_rel_nanos") {
      if (has_seen[15]) return false;
      has_seen[15] = true;
      int64 value;
      if (!parsed_colon || !::tensorflow::strings::ProtoParseNumericFromScanner(scanner, &value)) return false;
      msg->set_all_end_rel_nanos(value);
    }
    else if (identifier == "scheduled_nanos") {
      if (has_seen[16]) return false;
      has_seen[16] = true;
      int64 value;
      if (!parsed_colon || !::tensorflow::strings::ProtoParseNumericFromScanner(scanner, &value)) return false;
      msg->set_scheduled_nanos(value);
    }
  }
}
