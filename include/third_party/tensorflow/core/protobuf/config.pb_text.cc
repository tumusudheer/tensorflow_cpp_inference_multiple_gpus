...
void AppendProtoDebugString(
  ...
  o->AppendStringIfNotEmpty("executor_type", ProtobufStringToString(msg.executor_type()));
}
...
bool ProtoParseFromScanner(
  ...
  while(true) {
    ...
    else if (identifier == "executor_type") {
      if (has_seen[1]) return false;
      has_seen[1] = true;
      string str_value;
      if (!parsed_colon || !::tensorflow::strings::ProtoParseStringLiteralFromScanner(
          scanner, &str_value)) return false;
      SetProtobufStringSwapAllowed(&str_value, msg->mutable_executor_type());
    }
  }
}
...
void AppendProtoDebugString(
  ...
  o->AppendBoolIfTrue("use_run_handler_pool", msg.use_run_handler_pool());
}
...
bool ProtoParseFromScanner(
  ...
  while(true) {
    ...
    else if (identifier == "use_run_handler_pool") {
      if (has_seen[1]) return false;
      has_seen[1] = true;
      bool value;
      if (!parsed_colon || !::tensorflow::strings::ProtoParseBoolFromScanner(scanner, &value)) return false;
      msg->set_use_run_handler_pool(value);
    }
  }
}
